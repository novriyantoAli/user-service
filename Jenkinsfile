pipeline {
    ...
    environment {
        ...
        CONSUL_HTTP_KEY = credentials('consul-http-key')
        CONSUL_HTTP_TOKEN = credentials('consul-http-token')
        CONSUL_WATCH_INTERVAL_SECOND = credentials('consul-watch-interval-seconds')
    }
    ...
    stages {
        ...
        stage('Checkout Code') {
            steps {
                scripts {
                    def repoUrl = 'https://github.com/novriyantoAli/user-service.git'
        ...
        stage('Commit and Push Changes') {
            steps {
                script {
                    ...
                    git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@github.com/novriyantoAli/user-service.git
                    git add docker-compose.yaml
                    git commit -m 'Update image version to  ${TARGET_BRANCH}-${currentBuilder.number} [skip ci]' || echo 'No cnhange to commit'
        ...
        stage('Deploy and Remote Server') {
            steps {
                script {
                    def targetDir = "/home/rhein/conext/user-service"
                    ...
                    ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${USERNAME}@${HOST} '
                        if [ -d "${targetDir}/.git"]; then
                            ...
                            git clone -b "${TARGET_BRANCH}" git@github.com:novriyantoAli/user-service.git "${targetDir}"
                            cd "${targetDir}"
                        fi
